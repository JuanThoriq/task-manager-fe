...
const CustomAlert = ({
  ...
  const textColor =
    variant === "destructive" ? "text-red-800" : "text-violet-800";
  const borderColor =
    variant === "destructive" ? "border-red-200" : "border-violet-200";

  return (
    <div
      className={`flex items-start gap-3 rounded-lg border ${borderColor} ${bgColor} p-4`}
    >
      {icon}
...
export default function BoardPage() {
  ...
  async function fetchBoardDetail() {
    try {
      const res = await fetch(
        `https://localhost:5274/api/v1/boards/${boardId}`
      );
  ...
  async function handleAddCard() {
    ...
    try {
      ...
      const res = await fetch(
        `https://localhost:5274/api/v1/lists/${addCardListId}/cards`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
    ...
    <div className="p-6 pt-20 md:pt-24 max-w-[1600px] mx-auto">
      <AnimatePresence>
        {error && (
          ...
          >
            <CustomAlert
              variant="destructive"
              icon={<AlertCircle className="h-5 w-5 text-red-600 mt-0.5" />}
            >
              {error}
            </CustomAlert>
          </motion.div>
      ...
      >
        <div className="py-2">
          ...
          <label
            htmlFor="updateListOrder"
            className="text-sm font-medium text-gray-700 block mt-4 mb-2"
          >
            Order
          </label>
          <Input
      ...
      >
        <CustomAlert
          ...
          icon={<AlertCircle className="h-5 w-5 text-red-600 mt-0.5" />}
        >
          Are you sure you want to delete this list? All cards in this list will
